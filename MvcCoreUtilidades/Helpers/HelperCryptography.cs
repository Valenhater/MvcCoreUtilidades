...
namespace MvcCoreUtilidades.Helpers
{
    public class HelperCryptography
    {
        ...
        //CADA VEZ QUE REALICEMOS UN CIFRADO, SE GENERARA UN SALT DISTINTO
        private static string GenerateSalt()
        {
            ...
            for (int i = 1; i <= 50; i++)
            {   
                //UN NUMERO ENTRE LOS CARACTERES ASCI
        ...
        //VAMOS A CREAR UN METODO STATIC PARA CONVERTOR UN CONTENIDO Y CIFRADO Y DEVOLVEMOS EL TEXTO CIFRADO
        public static string EncriptarTextoBasico(string contenido)
        {
            //NECESITAMOS UN ARRAY DE BYTES PARA CONVERTIR EL TEXTO A byte[]
            byte[] entrada;
            //POSTERIORMENTE AL CIFRAR NOS DEVOLVERA UN ARRAY DE BYTES CON LA SALIDA CIFRADA
            byte[] salida;
            //NECESITAMOS UNA CLASE QUE MOS PERMITA CONVERTIR STRING A BYTE Y VICEVERSA
            UnicodeEncoding encoding = new UnicodeEncoding();
            //NECESITAMOS EL OBJETO PARA CIFRAR EL CONTENIDO
            SHA1Managed managed = new SHA1Managed();
            //SHA1 sha = sha.Create();
            //CONVERTIMOS EL STRING DE CONTENIDO A BYTE
            entrada = encoding.GetBytes(contenido);
            //EL OBJETO SHA RECIBE UN ARRAY DE BYTES E INTERNAMENTE MODIFICA CADA ELEMENTO DEVOLVIENDO OTRO ARRAY DE BYTES
            salida = managed.ComputeHash(entrada);
            //CONVERTIMOS DE NUEVO BYTES A STRING PARA HACERLO VISUAL
            string resultado = encoding.GetString(salida);
            return resultado;
        }
    }
